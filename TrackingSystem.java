...
public class TrackingSystem implements AgentInterface,SellerInterface,BuyerInterface{
    ...
    private<T extends TransactionInterface> List<T> filterTransaction(Predicate<Transaction> tPredicate){
    ...
    public List<BuyerTransaction> getTransactions(Buyer buyer){
        ...
        // return filterTransaction(t -> t.getBuyer().equals(buyer) && t.getUserStatus() != StatusForUser.CANCELLED);
    }
    ...
    public List<AgentTransaction> getTransactions(List<String> serviceLocations){
        ...
        // return filterTransaction(t-> t.getStatus() == TransactionStatus.INITIAL && serviceLocations.contains(t.getCurrentLocation().toLowerCase()));
    }
    ...
    public List<AgentTransaction> getTransactions(DeliveryAgent agent){
        List<AgentTransaction> atransactions = this.transactions.stream()
            ...
            .collect(Collectors.toList());
        return atransactions;
        // return filterTransaction(t -> (t.getStatus() == TransactionStatus.ONGOING && t.isCurrentAgent(agent)));
    }
